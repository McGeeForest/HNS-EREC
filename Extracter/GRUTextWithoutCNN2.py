...
class Extracter(nn.Module):
    def __init__(self, param):
        ...
        LSTMLayer.add_module('LSTM1', nn.LSTM(input_size=embed_dim, hidden_size=200, num_layers=1, bidirectional=True, batch_first=True))
        self.LSTMLayer = LSTMLayer

        SelfAttentionLayer = nn.Sequential()
        SelfAttentionLayer.add_module('SELF-ATT1', SelfAttention(400, 256))
        ...
        MFC.add_module('linear1', nn.Linear(400, out_dim))
...
#  初始化权值的方法，线性层使用xavier
    ...
    def forward(self, vec_batch):
        ...
        h_n = h_n.reshape(h_n.size()[0], -1)     # h_n.size()[0]是batch
        # print(h_n.size())
        # print(c_n.size())
        del vec_batch
        # x = self.MFC(h_n)
